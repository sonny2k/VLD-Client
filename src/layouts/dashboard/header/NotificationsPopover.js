    ...
    return (
      ...
      >
        ...
        <Scrollbar sx={{ height: { xs: 250, sm: 300, md: 350, lg: 450 } }}>
          ...
          >
            {[...notis].reverse().filter(notification => notification.seen === false).map((notification) => (
          ...
          >
            {[...notis].reverse().filter(notification => notification.seen === true).slice(0, position).map((notification) => (
              <NotificationItem key={notification._id} notification={notification} />
            ))}
          </List>
        </Scrollbar>       
...
function NotificationItem({ notification }) {
  ...
  return (
    <ListItemButton onClick={notification.type === "canceldoc" || notification.type === "confirm" ? () => seenUser(notification._id) : () => seenDoc(notification._id)}
      sx={{
        py: 1.5,
        px: 2.5,
        mt: '1px',
...
function renderContent(notification) {
  ...
  const title = (notification.type === "confirm" || notification.type === "canceldoc" ?
